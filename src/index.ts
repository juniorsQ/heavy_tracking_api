...
app.post('/create-test-data', async (req, res) => {
  try {
    ...
    const truckTypes = await prisma.truckType.createMany({
      data: [
        { name: 'Dump Truck', capacity: 10, description: 'Standard dump truck' },
        { name: 'Semi Trailer', capacity: 20, description: 'Large semi trailer' },
        { name: 'Flatbed', capacity: 15, description: 'Flatbed truck' },
        { name: 'Tanker', capacity: 12, description: 'Liquid tanker' }
      ],
      skipDuplicates: true
    });
    ...
    const workPlants = await prisma.workPlant.createMany({
      data: [
        { name: 'Central Plant', address: '123 Main St, Orlando', phone: '407-555-0101' },
        { name: 'North Plant', address: '456 North Ave, Tampa', phone: '813-555-0102' },
        { name: 'South Plant', address: '789 South Blvd, Miami', phone: '305-555-0103' }
      ],
      skipDuplicates: true
    });
